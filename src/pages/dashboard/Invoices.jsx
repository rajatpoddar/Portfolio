          ...
          {['all', ...Object.keys(STATUS_COLORS)].map((s) => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={`shrink-0 px-3 py-2 rounded-xl text-xs font-medium transition-all cursor-pointer capitalize ${
                statusFilter === s
                  ? 'bg-[#6c63ff]/20 border border-[#6c63ff]/40 text-white'
      ...
      <div className="glass rounded-2xl border border-white/5 overflow-hidden">
        {(() => {
          ...
          const renderCell = (inv, key) => {
            ...
            if (key === 'num') return <span className="font-mono text-xs text-white/40">{inv.invoice_number}</span>;
            if (key === 'client') return (
              <div>
                <div className="font-medium text-sm text-white/80">{client?.name || '—'}</div>
                <div className="text-xs text-white/35">{client?.business || ''}</div>
              </div>
            );
            if (key === 'project') return <span className="text-sm text-white/50">{inv.project_name || '—'}</span>;
            if (key === 'total') return <span className="font-bold text-sm text-white/80">₹{(inv.total_amount || 0).toLocaleString('en-IN')}</span>;
            if (key === 'due') return (
              <div>
            ...
            <>
              {inv.status !== 'paid' && (
                <button onClick={() => handleMarkPaid(inv)}
                  className="btn-icon px-2.5 py-1 rounded-lg bg-green-500/10 hover:bg-green-500/20 text-green-400 text-xs font-medium transition-colors cursor-pointer">
                  ✓ Paid
                </button>
              )}
              <button onClick={() => setPreview(inv)}
                className="btn-icon w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center text-white/40 hover:text-white text-xs transition-colors cursor-pointer">👁</button>
              <button onClick={() => setModal(inv)}
                className="btn-icon w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center text-white/40 hover:text-white text-xs transition-colors cursor-pointer">✏️</button>
              <button onClick={() => handleDelete(inv.id)}
                className="btn-icon w-8 h-8 rounded-lg bg-red-500/5 hover:bg-red-500/15 flex items-center justify-center text-red-400/50 hover:text-red-400 text-xs transition-colors cursor-pointer">🗑</button>
            </>
          );
